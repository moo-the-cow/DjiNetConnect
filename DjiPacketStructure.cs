public static class DjiPacketStructure
{
    ...
    public static byte[] BuildDjiFrame(byte[] command, byte[] id, byte[] type, byte[] data)
    {
        ...
        payload[3] = DjiCrc.Crc8(firstThreeBytes); // Now uses reflection
        
        // Calculate and append CRC16 with reflection
        byte[] crc16 = DjiCrc.Crc16(payload.ToArray()); // Now uses reflection
